...
import json
import torch
from apps.home.nltk_utils import tokenize, bag_of_words, detect_language
from apps.home.model import NeuralNet
...
with open('./apps/home/data/french/random_mess.json', 'r', encoding='utf-8') as f:
    messages_fr = json.load(f)

with open('./apps/home/data/french/remember.json', 'r') as f:
    remember_fr = json.load(f)

with open('./apps/home/data/english/random_mess_en.json', 'r', encoding='utf-8') as f:
    messages_en = json.load(f)

with open('./apps/home/data/english/remember_en.json', 'r') as f:
    remember_en = json.load(f)

with open('./apps/home/data/full/intents.json', 'r', encoding='utf-8') as f:
    start = time.time()
    intents = json.load(f)
    end = time.time()
    print(f'Time taken to load json file: {end-start:.5f} seconds')

FILE = "./apps/home/model/data.pth"
try:
